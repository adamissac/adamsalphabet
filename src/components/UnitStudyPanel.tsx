...
/**
 * Left study guide — pushes main content when open. The panel itself is
 * sticky so it stays on screen while you scroll through the unit.
 */
  ...
  return (
    <div
      className={[
        ...
        "transition-[grid-template-columns] duration-300 ease-[cubic-bezier(0.22,0.9,0.3,1)]",
    ...
    >
      {/* Sticky sidebar — follows you down the page as you scroll */}
      <aside
        aria-label="Study guide"
        aria-hidden={!open}
        className={[
          "sticky top-16 z-10 min-w-0 self-start",
      ...
      >
        <div className="flex items-center justify-between gap-3 px-4 py-4 border-b border-[var(--color-border)]">
          ...
          <button
            ...
            className="w-9 h-9 rounded-full grid place-items-center text-[var(--color-ink-muted)] hover:bg-[var(--color-surface-2)] hover:text-[var(--color-ink)] transition-colors flex-shrink-0"
              ...
              <ul className="grid gap-0.5">
                {navLinks.map((link) => (
                  <li key={link.href}>
                    <a
                      href={link.href}
                      className="block px-3 py-2 rounded-md text-sm text-[var(--color-ink-muted)] hover:text-[var(--color-brand-700)] hover:bg-[var(--color-surface-2)] transition-colors"
                    >
                      {link.label}
                    </a>
                  </li>
      ...
      {/* Main content — shifts right when the sidebar opens */}
      <div className="min-w-0">
        {!open && (
